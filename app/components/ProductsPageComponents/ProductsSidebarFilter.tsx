...
export default function ProductsSidebarFilter({
    ...
    const categoriesSection = categories.length > 0 && (
        <div className="space-y-2.5 pt-2 border-t border-slate-100 dark:border-white/10">
            ...
            <div className="max-h-48 overflow-y-auto space-y-1 pe-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-zinc-700 lg:max-h-none lg:overflow-visible">
                {categories.map((cat) => {
        ...
        <div className="space-y-2.5 pt-2 border-t border-slate-100 dark:border-white/10">
            ...
            {brands.length > 5 && (
                <div className="relative">
                    <Search className="absolute start-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm" />
                    <input
                        type="text"
                        value={brandSearch}
                        ...
                        className="w-full ps-8 pe-2.5 py-1.5 text-xs bg-slate-50 dark:bg-zinc-800/80 border border-slate-200 dark:border-white/10 rounded-lg focus:outline-none focus:border-[#8A6305] text-[#0B192C] dark:text-white placeholder:text-slate-400 transition-colors"
                    />
                    {brandSearch && (
                        <button
                            type="button"
                            onClick={() => setBrandSearch("")}
                            className="absolute end-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-white"
                        >
                            <X className="text-xs" />
                        </button>
                    )}
            ...
            {/* Brand List */}
            <div className="max-h-56 overflow-y-auto space-y-1 pe-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-zinc-700 lg:max-h-none lg:overflow-visible">
    ...
    return (
        <>
            ...
            {isMobileDrawerOpen && (
                <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end bg-black/55 animate-in fade-in duration-200">
                    ...
                    <div
                        ...
                        className="bg-white dark:bg-zinc-900 rounded-t-2xl p-5 max-h-[88vh] flex flex-col border-t border-slate-200 dark:border-white/10 animate-in slide-in-from-bottom duration-300"
                    >
                        {/* Drawer Grab Bar */}
                        <div className="w-12 h-1 bg-slate-300 dark:bg-zinc-700 rounded-full mx-auto mb-3" />

                        {/* Top close bar */}
                        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-white/10 mb-4">
                            <span id="catalog-filter-title" className="font-bold text-base text-[#0B192C] dark:text-white">
                                {isArabic ? "تصفية المنتجات" : "Filter Catalog"}
                            </span>
                            <button
                                type="button"
                                ...
                                className="w-11 h-11 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-slate-500 hover:text-slate-800 dark:hover:text-white cursor-pointer"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="pt-4 mt-2 border-t border-slate-100 dark:border-white/10">
                            <button
                                type="button"
                                onClick={onCloseMobileDrawer}
                                className="w-full py-3 bg-[#0B192C] dark:bg-white dark:text-slate-900 text-white font-bold rounded-xl text-sm transition-colors cursor-pointer"
                            >
                                {isArabic
