...
const OrderShippingAndPayment = ({ shopName, name, streetAddress, city, phone, notes, isQuoteRequest = false }: OrderShippingAndPaymentProps) => {
        ...
        <div className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 gap-8 text-center md:ltr:text-left md:rtl:text-right">
            <div className="flex flex-col gap-3 items-center md:items-start">
                <h3 className="text-sm font-extrabold flex items-center gap-2 text-[#0B192C] dark:text-white">
                    <Truck className="text-[#0B192C] dark:text-[#8A6305] text-lg" />
            ...
            <div className="flex flex-col gap-3 items-center md:items-start">
                <h3 className="text-sm font-extrabold flex items-center gap-2 text-[#0B192C] dark:text-white">
                    <CreditCard className="text-[#0B192C] dark:text-[#8A6305] text-lg" />
                    {isQuoteRequest
                        ? (language === 'ar' ? 'ترتيبات الدفع' : 'Payment terms')
                        : t('checkout.paymentMethod')}
                </h3>
                <div className="flex flex-col gap-0.5 w-full">
                    <p className="text-xs sm:text-sm font-bold text-[#0B192C] dark:text-white">
                        {isQuoteRequest
                            ? (language === 'ar' ? 'تؤكد مع قسم المبيعات' : 'Confirmed with sales')
                            : t('checkout.cashOnDelivery')}
                    </p>
                    <p className="text-xs text-gray-400">
                        {isQuoteRequest
                            ? (language === 'ar'
                                ? 'لا يتم تحصيل أي دفعة الآن'
                                : 'No payment is collected now')
                            : (language === 'ar'
                                ? 'تسوية نقدية عند الاستلام بموجب فاتورة الوكالة الرسمية'
                                : 'Settlement upon delivery per official agency invoice')}
                    </p>
                </div>
            </div>
        </div>
