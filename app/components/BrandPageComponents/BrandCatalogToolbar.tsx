        ...
        <div className="mb-6 space-y-3 sm:space-y-4" aria-label="Catalog Filters">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/80 dark:border-white/10">
                ...
                <div className="relative flex-1 max-w-md">
                    <Search className={`absolute top-1/2 -translate-y-1/2 text-slate-400 text-lg ${isRtl ? "right-3" : "left-3"}`} />
                    <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => onSearchChange(e.target.value)}
                        placeholder={isArabic ? `ابحث في منتجات ${brandShortName}...` : `Search in ${brandShortName} items...`}
                        className={`w-full h-10 bg-slate-50 dark:bg-zinc-900 border border-slate-200/90 dark:border-white/10 rounded-xl text-xs sm:text-sm text-[#0B192C] dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-[#8A6305] focus:ring-0 transition-colors ${
            ...
            {hasCategories && (
                <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto hide-scrollbar py-1" style={{ scrollbarWidth: "none" }}>
                    <button
                        ...
                        className={`shrink-0 inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
                            activeCategoryId === "all"
                                ? "bg-[#0B192C] dark:bg-[#8A6305] text-white"
                                : "bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-[#8A6305]"
                        }`}
                    >
                    ...
                    {categories.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                className={`shrink-0 inline-flex items-center px-3.5 py-1.5 rounded-lg text-xs font-bold transition-colors cursor-pointer ${
                                    isSelected
                                        ? "bg-[#0B192C] dark:bg-[#8A6305] text-white"
                                        : "bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-[#8A6305]"
                                }`}
